...
![Parameters of LLMs](collective-communication-starter-0/estimated-capability-density-for-open-source-base-LLMs.png)
...
![scaling](collective-communication-starter-0/scale-up-vs-scale-out-011675.png)
...
通信瓶颈可以从两个最基础的量理解：延迟（Latency）和带宽（Bandwidth）。

延迟描述一条消息从发送到接收所需的固定开销。它和消息大小关系不大，小消息尤其怕延迟。带宽描述单位时间能传输多少数据，大消息更容易受带宽限制。
...
其中`alpha`代表启动开销或链路延迟，`size / bandwidth`代表数据真正被搬运所需的时间。对于多轮通信算法，可以把它扩展成：

``` plain text
T = steps * alpha + bytes / bandwidth
```

这就是为什么集合通讯算法要关心拓扑。比如Ring AllReduce可以很好地利用带宽，但是需要多轮传输；Tree算法的通信轮数更少，小消息延迟表现更好，但可能无法像Ring一样稳定打满每条链路。

除了延迟和带宽，还有几个实际系统中很重要的因素。

1. **拓扑非均匀**：不是任意两张GPU之间的通信代价都一样。跨NVLink、跨PCIe Switch、跨NUMA、跨节点的性能可能差很多。
2. **带宽收敛**：多个流量共享同一条上行链路时，理论带宽不能简单相加。
3. **拥塞与路由**：Scale-Out之后，交换机和网络路由会显著影响尾延迟。
4. **协议栈开销**：TCP/IP、RDMA、GPU Direct RDMA等路径的CPU参与程度不同。
5. **计算通信重叠**：即使通信本身没有变快，只要能被计算隐藏，端到端性能也会提升。

从这个角度看，集合通讯不是简单的“数据从A到B”，而是一个在硬件拓扑上做调度的问题。

# 大规模网络

理解完分布式计算为什么需要通信之后，我们再看通信本身如何组织。
...
![Topology](collective-communication-starter-0/An-overview-of-basic-types-of-network-topologies-including-the-A-chain-or-line-B.webp)
...
现代数据中心节点间连接一般使用Tree，节点内GPU通讯使用Mesh。
